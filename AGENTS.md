...
- `shaders/` — Slang sources in `slang/`, compiled `.spv` (gitignored, not committed) + `reflection/*.json` next to them.
...
- Shaders are **Slang** (`shaders/slang/*.slang`), compiled with `slangc` to `.spv`.
  ...
  Compile via `bash assets/shaders/compile.sh` (or `shaders/compile.bat` on Windows). Requires `slangc` on PATH.
...
- Reflection JSON (`shaders/reflection/*.json`) is committed and drives `xtask` Rust binding codegen.
