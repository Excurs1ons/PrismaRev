//! Asset decoders — G1 预留实现（DESIGN §10.11）
//!
...
/// 解析 RTEX 字节 → TextureUploadInput（支持 RGBA8 与压缩块预留）
pub fn decode_rtex(bytes: &[u8]) -> Result<TextureUploadInput> {
    ...
    // header: [magic 4][w 4 LE][h 4 LE][mip 1][fmt 1] ... 简化预留
    if bytes.len() >= 14 {
