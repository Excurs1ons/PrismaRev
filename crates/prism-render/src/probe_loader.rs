//! Baked GI probe-volume binary loader/saver.
//!
//! Migrated here from the legacy dev-mode `prism-asset` crate so the GI
//! runtime has zero dependencies on that crate. The on-disk format is
//! unchanged (PRPV magic, version 2); existing `.bin` files continue to load.
//!
...
/// Baked GI probe volume in CPU memory.
///
