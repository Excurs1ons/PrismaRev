...
- **布局**：底部横排 6 个模式按钮（FINAL / ALBEDO / SPEC / REFL / AMBIENT / NORMAL）；选中高亮；NORMAL 选中时显示 `W/V/T` 空间指示。按钮矩形与文字方块由同一布局函数生成（overlay 绘制与命中测试共用）。
- **命中测试**：按钮矩形以像素、左上原点定义（与指针坐标同空间）；overlay 着色器内将像素坐标转 clip 空间（注意 y 翻转）。
