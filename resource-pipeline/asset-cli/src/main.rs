...
fn cmd_scan(root: &Path, _cli: &Cli) -> anyhow::Result<()> {
    ...
    if !assets.exists() {
        anyhow::bail!("Assets/ directory not found / 目录不存在. Run 'asset-cli init' first.");
    }
...
fn cmd_build(
    ...
    if !db_path(root).exists() {
        anyhow::bail!("No AssetDatabase.json found / 未找到数据库. Run 'asset-cli scan' first.");
    }

    let db = AssetDatabase::load(&db_path(root))?;
    if db.is_empty() {
        anyhow::bail!("No assets to build / 没有可构建的资源. Run 'asset-cli scan' first.");
    }
    ...
    if let Ok(reader) = asset_package::PackageReader::from_bytes(&pak) {
...
fn cmd_validate(pak_path: &Path) -> anyhow::Result<()> {
    ...
    if meta_assets.is_empty() {
        println!();
        println!("💡  Tip / 提示:");
        println!("     Rebuild with current asset-cli to generate .pak.meta.json");
